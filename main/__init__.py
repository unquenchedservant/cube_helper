...
def screen(stdscr):
	...
	if utilities.update_available():
		options = ["Scramble Generator", "2-Look OLL Helper", "2-Look PLL Helper", "Download and Install Update"]
	else:
	...
	while True:
		...
		if k == 27 or k == ord('q'):
			...
			if y_pos == 1:
				scramble.screen(stdscr)
			elif y_pos == 2:
				oll.screen(stdscr)  # oll helper
			elif y_pos == 3:
				pll.screen(stdscr)
			elif y_pos == 4 and utilities.update_available():
			...
			pll.screen(stdscr)
		elif k == ord('4') and utilities.update_available():
			update.screen(stdscr)
